...
__forceinline__ __device__ void getRect(const float2 p, int2 radius, uint2& rect_min, uint2& rect_max, dim3 grid)
{
	rect_min = {
		min(grid.x, max((int)0, (int)((p.x - radius.x) / BLOCK_X))),
		min(grid.y, max((int)0, (int)((p.y - radius.y) / BLOCK_Y)))
	};
	rect_max = {
		min(grid.x, max((int)0, (int)((p.x + radius.x + BLOCK_X - 1) / BLOCK_X))),
		min(grid.y, max((int)0, (int)((p.y + radius.y + BLOCK_Y - 1) / BLOCK_Y)))
	};
}
