	...
	bool prefiltered)
{
	...
	int2 radius;
	{
		...
		radius.x = int(3 * ceil(extent.x));
		radius.y = int(3 * ceil(extent.y));
	}
	...
	//radii[idx * 2 + 1] = 5;
	radii[idx * 2] = radius.x;
	radii[idx * 2 + 1] = radius.y;
	...
	float* __restrict__ max_weight)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float w = alpha * T;
		    if (w > max_weight[collected_id[j]])
			{
			    max_weight[collected_id[j]] = w;
			}
#if RENDER_AXUTILITY
