...
class GaussianModel:
    ...
    def prune_points(self, mask):
        valid_points_mask = ~mask # since  ~mask -> valid_points_mask is True when the point must be mantain, False if must be removed
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        selected_pts_mask = torch.logical_and(selected_pts_mask, torch.max(self.get_scaling, dim=1).values > self.percent_dense*scene_extent) # selected points must have sufficient scale
